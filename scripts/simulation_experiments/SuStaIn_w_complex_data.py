...
def run_SuStaIn(run_path):
    ...
    output_dir = os.path.join(write_dir, "{}_samples".format(n_samples), "{}_dims".format(dim), "{}_components".format(nc), "SuStaIn_classic")
    output_folder = os.path.join(output_dir, dataset_name)
    if not os.path.isdir(output_folder):
        os.makedirs(output_folder)

    sustain                 = MixtureSustain(L_yes, L_no, SuStaInLabels, N_startpoints, N_S_max, N_iterations_MCMC, N_em, output_folder, dataset_name, use_parallel_startpoints)
