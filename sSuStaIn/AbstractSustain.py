...
class AbstractSustain(ABC):
    ...
    def _estimate_ml_sustain_model_nplus1_clusters(self, sustainData, ml_sequence_prev, ml_f_prev):
        ...
        if N_S == 1:
            # print("Stage Sizes", np.cumsum(sustainData.stage_size[:-1]))
    ...
    def _find_ml(self, sustainData):
        ...
        # ml_sequence_mat                     = np.zeros((1, sustainData.getNumBiomarkers(), self.N_startpoints))
        ml_sequence_list                    = []
    ...
    def _find_ml_split(self, sustainData):
        ...
        ml_sequence_mat                     = []
        # ml_sequence_mat                     = np.zeros((N_S, sustainData.getNumBiomarkers(), self.N_startpoints))
    ...
    def _find_ml_split_iteration(self, sustainData, seed_seq):
        ...
        # seq_init                            = np.zeros((N_S, sustainData.getNumBiomarkers()))
        seq_init                            = []
        for s in range(N_S):
