...
class sEBMSustainData(AbstractSustainData):
    ...
    def __init__(self, L_yes, L_no, stage_size, bm_select = True):
        ...
        assert type(bm_select) == bool
        self.bm_select = bm_select
        if bm_select:
            numStages = len(stage_size) - 1
        else:
    ...
    def reindex(self, index, stage_size_custom):
        return sEBMSustainData(self.L_yes[index,], self.L_no[index,], stage_size_custom)
...
class sEBMSustain(AbstractSustain):
    ...
    def __init__(self,
        ...
        self.__sustainData              = sEBMSustainData(L_yes, L_no, stage_size, bm_select)
        self.stage_size                 = stage_size
    ...
    def _calculate_likelihood_stage(self, sustainData, S):
        ...
        bm_select = sustainData.bm_select
        assert len(ss) - bm_select == N
